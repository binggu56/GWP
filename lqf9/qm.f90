      program main
!     -------------------------------------------------
!     trajectory-guided guassian basis
!     with trajectories moving as quantum trajectories 
!     with linear quantum force 
!     ---------------------------------------------------
      use cdat
...
!      complex*16,allocatable :: c(:),c0(:),dc(:)
      ...
      open(110,file='c0.dat')
      open(111,file='norm')
      open(112,file='force')
      open(117,file='xc')
      open(118,file='cnum')
      open(119,file='rep.dat')
...
!     test distribution
      ...
      do i=1,Ntraj
        ...
        a1 = a1+w(i)*x(i)
      enddo
      write(*,*) 'norm w & expec of x',an,a1
      write(*,*) 'grid range',xl,xr,gx
...
!     print out basis functions
      do i=1,Np 
        xi = xmin+dx*dble(i-1)
        z0 = dsqrt(dsqrt(al/pi))*exp(-al/2d0*(xi-x(1))**2+im*p(1)*(xi-x(1)))
        zn = dsqrt(dsqrt(al/pi))*exp(-al/2d0*(xi-x(nb))**2+im*p(nb)*(xi-x(nb)))
...
!      call rep(nb,w,r0,x,p,dt,dxp)
!      do i=1,nb
!        write(119,10000) x(i),dxp(i)
!      enddo
...
!------check normalization--------------
      ...
      do i=1,Np
        ...
        if(abs(psi(i)) .gt. 1d-3) write(104,10000) xi,abs(psi(i))**2/prob
      enddo
      write(*,1013)  prob
...
!          call reduce(nb,w,x,p,c,cutoff)
        
!          call reduce(nb,w,x,p,c)
!          call rep(nb,w,r0,x,p,dt,dxp)
          call derivs(x,Nb,v,dv,ddv)
          call qpot(Nb,ddv,c,am,p,x,w,u,du)
...
10000  format(80(e14.7,1x))
      end program main


!-----------repulsion of trajectories--------------------------------------------------
! force between trajectories exp(-abs(r-r0))/(r-r0)
! with r0 determined by initial value of r (or maybe by corressponding harmonic
!-----------------------------------------------------------------------------------
!      subroutine rep(nb,w,r0,x,p,dt,dxp)
!      use cdat
!      implicit real*8(a-h,o-z)
!      real*8 x(nb),p(nb),dxp(nb),r0(nb,nb),w(nb)
!!      a = 6d0
!      ww = 0d0
!      do i=1,nb
!        ww = w(i)+ww
...
!     ---------------------------------------------
      ...
      do i=1,Np
        ...
        psi = (0d0,0d0)
        do j=1,nb
          psi = psi+c(j)*exp(-al/2d0*(xi-q(j))**2+im*p(j)*(xi-q(j)))*dsqrt(dsqrt(al/pi))
        enddo
...
!     ------------------------------------------
      ...
      subroutine increm(t,nb,mat,h,c,dc)
      use cdat
      implicit real*8(a-h,o-z)
      ...
      real*8     d(nb,nb)
      complex    :: SWORK(nb*(nb+1))
      real*8     :: RWORK(Nb)
      complex*16 :: WORK(Nb,1)
      integer    :: INFO,ITER
!---------H*c---------------
      do i=1,Nb
        b(i) = (0d0,0d0)
...
!      write(118,1000) t,cnum
!      if(cnum .gt. 1.d08) then
!        write(*,*) 'Condition number too big.'
!        stop
!      endif

      call zposv('U', Nb, 1, aux, nb, b, nb, INFO)
!      call ZCPOSV('U',Nb,1,aux,nb,b,nb,dc,nb,WORK,SWORK,RWORK, ITER, INFO)
...
!     ----------------------------------------------
      ...
      complex*16 :: z,z0,mat(Ntraj,Ntraj),psi0,aux(nb,nb),c(nb,1)
      real*8     :: d(nb,nb)
      complex    :: SWORK(nb*(nb+1))
      real*8     :: RWORK(Nb)
      complex*16 :: WORK(Nb,1)
...
!      call zposv('U', Nb, 1, aux, nb, c0, nb, INFO)
      call ZCPOSV('U',Nb,1,aux,nb,c0,nb,c,nb,WORK,SWORK,RWORK, ITER, INFO)
