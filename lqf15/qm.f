...
!----------------------------------------------------------------------
     ...
     &                      ddv(:),s(:)
      ...
      open(114,file='t100.dat')
      open(115,file='t300.dat')
      open(116,file='t500.dat')
      open(117,file='t600.dat')
...
!     test distribution
      ...
      write(*,*) an,a1
      write(*,1233) xl,xr,gx
1233  format('xrange = [', f10.6,',',f10.6,'] ','dx = ',f10.6)
      ...
      if(k == 500) then
        do i=1,np
          ...
          if(abs(psi(i)) .gt. 1d-3) then 
            write(114,10000) xi,abs(psi(i))**2/prob
          endif
        enddo
      elseif(k == 1500) then
        do i=1,np
          xi = xmin+dx*(i-1)
          if(abs(psi(i)) .gt. 1d-3) then 
            write(115,10000) xi,abs(psi(i))**2/prob
          endif
        enddo
      elseif(k == 2500) then
        do i=1,np
          xi = xmin+dx*(i-1)
          if(abs(psi(i)) .gt. 1d-3) then 
            write(116,10000) xi,abs(psi(i))**2/prob
          endif
        enddo
        elseif(k==3000) then
        do i=1,np
          xi = xmin+dx*(i-1)
          if(abs(psi(i)) .gt. 1d-3) then 
            write(117,10000) xi,abs(psi(i))**2/prob
          endif
        enddo
      endif
...
!-----potential------------------------
      ...
      real*8,intent(OUT) :: v(Nb),dv(Nb),ddv(Nb)
...
!----------------------------------------------
      ...
      do i=1,Nb
        v(i) = aw*x(i)**4-bw*x(i)**2
        dv(i) = 4d0*aw*x(i)**3-2d0*bw*x(i)
        ddv(i) = 12d0*aw*x(i)**2-2d0*bw
      enddo
